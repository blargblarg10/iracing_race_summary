...
ACCEPT_TYPES = {
                ...
                "Average Race Pace"        : "average_lap_time",
...
def initialize_api_client(cfg_file):
...
def fetch_race_data(client, series_name, season_year, season_quarter, race_week_num):
    '''
    ...
    df_filename = f"{season_year}_{season_quarter}_{race_week_num}_{series_name}.pkl"
    ...
    new_data = []  # List to collect new data
    total_sessions = len(races)

    for index, session in enumerate(races):
        subsession_id = session['subsession_id']
        # Skip if session data already gathered
        if subsession_id not in df['subsession_id'].values:
            # Get session data 
            session_results = client.result(subsession_id=subsession_id, include_licenses=True)

            # Add MINOR revisions to API results by adding all the laps in a session and getting average 5, clean laps, and total laps for each driver
            summary, laps = preprocess_lap_data(client, subsession_id)
            session_results["race_laps_TSS"] = laps
            merge_summary_and_session(summary, session_results)
            session_results = correct_race_qual_column(session_results)

            # Append data
            new_data.append(session_results)

        print(f'Progress: {(index + 1) / total_sessions * 100:.2f}%', end='\r', flush=True)

    # Append new data to the DataFrame outside of the loop
    if new_data:
...
def time_irating_preprocessing(df):
    # Step 1: Isolate 'subsession_id' and the relevant part of 'session_results'
    
    subsession_ids = df['subsession_id']
    sof = df['event_strength_of_field']
    nested_data = df['session_results'].apply(lambda x: x[2]['results'])
    ...
    flattened_df = pd.concat(temp_dfs, ignore_index=True)
    return flattened_df.copy()


#Estimate Lap times
def estimate_best_lap(lowess_result, irating):
...
def validate_scatter_type(type, rating_type):
    if type in ACCEPT_TYPES and rating_type in RATING_TYPES:
        return ACCEPT_TYPES[type], RATING_TYPES[rating_type]
    else:
        print(f"{type} is invalid")
        quit()


def time_df_binning(df, time_column, rating_colum):
    bin_edges = list(range(0, 10001, 200))
...
def plot_scatter(scatter_df, highlight_driver = [], type = "", rating_type= "iRating", position_mask = None,irating = [1000, 1500, 2000, 3000], clip=False):
    time_column, rating_colum = validate_scatter_type(type, rating_type)
    # Converts iracing time to ms and removes anything 
    df = scatter_df.loc[(scatter_df[time_column] >= 0)]
    hsuh = df.to_dict()
    if position_mask:
        df = df.loc[(scatter_df['starting_position'] <= position_mask)]
    df.loc[:, time_column] = df.loc[:, time_column]/10000

    # Bin the irating together to apply a color range per bin instead of total time
    df = time_df_binning(df, time_column, rating_colum)
    ...
    ax2.set_title(f'{type} Time vs {rating_type}')
    ...
    estimated_times = [estimate_best_lap(lowess_fitted, ir) for ir in irating]
...
def main():
    print("Starting")
    series_name = "iRacing Porsche Cup By Coach Dave Delta - Fixed"
    # series_name = "FIA Formula 4 Challenge"
    # series_name = "FIA Formula 4 Challenge - Fixed"
    # series_name = "Formula C - Super Formula Lights"
    # series_name = "Formula C - Super Formula Lights - Fixed"
    ...
    initialize_api_client(cfg_file="/mnt/c/Users/tssom/OneDrive/Documents/Python Scripts/iracing_race_summary/credentials.cfg")
    race_df = fetch_race_data(series_name, season_year, season_quarter, race_week_num)

    # Preprocess data for time v irating plots
    scatter_df = time_irating_preprocessing(race_df)

    plot_scatter(scatter_df.copy(), highlight_driver=DRIVERS, type="Fastest Lap")
    plot_scatter(scatter_df.copy(), highlight_driver=DRIVERS, type="Qualify", clip=True)
    plot_scatter(scatter_df.copy(), highlight_driver=DRIVERS, type="Qualify", rating_type="SOF", position_mask=5)
